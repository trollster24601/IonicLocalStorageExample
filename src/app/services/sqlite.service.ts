...
export class SQLiteService {
  ...
  private apiUrl = 'http://172.22.6.17/todo/api.php';
  ...
  private async createTable(): Promise<void> {
    ...
    await this.db.run({database:"todo_db", statement: query, values: [] });
  }

  async addTodo(taskName: string): Promise<void> {
    const query = 'INSERT INTO todo (task_name) VALUES (?)';
    await this.db.run({database:"todo_db", statement: query, values: [taskName] });
  }

  async getTodos(): Promise<any[]> {
    const query = 'SELECT * FROM todo';
    const result = await this.db.query({database:"todo_db",  statement: query, values: [] });
    return result?.values || [];
  }

  async updateTodoStatus(id: number, completed: number): Promise<void> {
    const query = 'UPDATE todo SET completed = ? WHERE id = ?';
    await this.db.run({database:"todo_db",  statement: query, values: [completed, id] });
  }

  async deleteTodo(id: number): Promise<void> {
    const query = 'DELETE FROM todo WHERE id = ?';
    await this.db.run({database:"todo_db",  statement: query, values: [id] });
  }
